...
### Added
- Added Spark and AgentSeal README badges.
- Added concise MCP tool docstrings to improve agent tool selection metadata.

### Fixed
- Normalized MediaWiki coordinate marker fields so `primary` remains a boolean in MCP tool output.
- Ensured the release workflow checks out the repository before reading `CHANGELOG.md` for release notes.

### Removed
- Removed tracked virtual environment artifacts, unused Node/Playwright manifests, and unreferenced release payload data from the repository.
