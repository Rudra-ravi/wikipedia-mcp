...
def create_server(
    ...
    def summarize_article_for_query(
        ...
        logger.info(
            f"Tool: Getting query-focused summary for article: {title}, query: {query}"
        )
    ...
    def summarize_article_section(
        ...
        logger.info(
            f"Tool: Getting summary for section: {section_title} in article: {title}"
        )
    ...
    def extract_key_facts(
        title: str,
        topic_within_article: Annotated[
            str, Field(title="Topic Within Article")
        ] = "",
        count: int = 5,
    ) -> Dict[str, Any]:
        """Extract key facts from a Wikipedia article, optionally focused on a topic."""
        logger.info(
            f"Tool: Extracting key facts for article: {title}, topic: {topic_within_article}"
        )
    ...
    def summary_for_query_resource(
    ...
    def key_facts_resource(
        title: str, topic_within_article: str, count: int
    ) -> Dict[str, Any]:
